...
def convert_hpxml_and_parse(input_filename, version=4):
...
def test_enclosure_foundation():
    ...
    for i in (0, 1):
        fw1 = root.Building[i].BuildingDetails.Enclosure.FoundationWalls.FoundationWall[0]
        assert not hasattr(fw1, 'DistanceToTopOfInsulation')
        assert not hasattr(fw1, 'DistanceToBottomOfInsulation')

        fw2 = root.Building[i].BuildingDetails.Enclosure.FoundationWalls.FoundationWall[1]
